...
class FusionAdapter(nn.Module):
    ...
    """
    def __init__(self, channels, num_modalities=4, cond_dim=32):
...
class Decoder_fuse(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.adapter = FusionAdapter(basic_dims, num_modalities=4)

    def forward(self, x1, x2, x3, x4, mask):
        # x1 - torch.Size([1, 4, 16, N, H, W])
        # x2 - torch.Size([1, 4, 32, N/2, H/2, W/2])
        # x3 - torch.Size([1, 4, 64, N/4, H/4, W/4])
        # x4 - torch.Size([1, 4, 128, N/8, H/8, W/8])
        # mask - torch.Size([B, 4])
        ...
        de_x4 = self.d3_c1(self.up2(de_x4))         # torch.Size([1, 64, 28, 28, 24])
        ...
        de_x3 = self.d2_c1(self.up2(de_x3))                 # torch.Size([1, 32, 56, 56, 48])
        ...
        de_x2 = self.d1_c1(self.up2(de_x2))                 # torch.Size([1, 16, 112, 112, 96])
        ...
        logits = self.seg_layer(fused)      # torch.Size([1, 4, 112, 112, 96])
...
class E4DModel(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        fuse_pred, prm_preds, fusion_preds, _ = self.decoder_fuse(x1, x2, x3, x4, mask)
...
class E4D4Model(nn.Module):
    ...
    def encode(self, x):
        ...
        per_modal = (
            (c1_x1, c1_x2, c1_x3, c1_x4),
            (c2_x1, c2_x2, c2_x3, c2_x4),
            (c3_x1, c3_x2, c3_x3, c3_x4),
            (c4_x1, c4_x2, c4_x3, c4_x4),
        )
        return x1, x2, x3, x4, per_modal

    def decode(self, x1, x2, x3, x4, mask):
        return self.decoder_fuse(x1, x2, x3, x4, mask)

    def forward(self, x, mask, fx1=None, fx2=None, fx3=None, fx4=None):
        # fx1..fx4 are accepted (and ignored) only so callers written for the
        # older anchor-conditioned decoder keep working unchanged.
        x1, x2, x3, x4, per_modal = self.encode(x)

        fuse_pred, prm_preds, fusion_preds, fused_repr = self.decode(x1, x2, x3, x4, mask)

        if self.is_training:
            flair_pred = self.decoder_sep(*per_modal[0])
            t1ce_pred = self.decoder_sep(*per_modal[1])
            t1_pred = self.decoder_sep(*per_modal[2])
            t2_pred = self.decoder_sep(*per_modal[3])
...
if __name__ == "__main__":
    ...
    model = E4D4Model(num_cls=4).cuda()
