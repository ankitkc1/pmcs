...
# 蒸馏损失——https://blog.csdn.net/DD_PP_JJ/article/details/121900793
...
def msa_loss(output, target, num_cls=3): #
    
    assert output.shape == target.shape
    P = output.shape[0] * output.shape[1]
    Lc = torch.pow(torch.sum(target-output)/P, 2)
