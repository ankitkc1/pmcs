...
from utils.fl_utils import apply_morphological_operations
import numpy as np
...
class Brats_train(Dataset):
    def __init__(self, transforms='', root=None, modal='all', num_cls=4, train_file='', all_ = False, partial = 1):
        ...
        volpaths = []
        files = glob.glob(os.path.join(root, "vol", "*.npy"))
        for dataname in datalist:
            for file in files:
                if dataname in file:
                    volpaths.append(file)

        self.volpaths = volpaths    # 获得所有的vol数据路径
    ...
    def __getitem__(self, index):
        ...
        x,y = self.transforms([x, y])   # 随机裁剪到80*80*80
        ...
        # yo - (1, 4, 80, 80, 80) onehot编码的分割标注

        x = x[:, self.modal_ind, :, :, :]   # 取指定模态的vol数据
...
class Brats_test(Dataset):
    def __init__(self, transforms='', root=None, modal='all', test_file='test.txt', all_ = False):
        ...
        if not all_:
            print(datalist)
            datalist = datalist[int(len(datalist) * 0.6):]
    ...
    def __getitem__(self, index):
        ...
        # x - torch.Size([1, H, W, D])
        # y - torch.Size([146, 164, 138])
        return x, y, name
...
class GLB_Brats_train(Dataset):
    def __init__(self, transforms='', root=None, modal='all', num_cls=4, train_file='train.txt', all_ = False, partial = 1, aug = False):
        self.aug = aug
        ...
        for l in range(len(datalist)):
            self.random_idx.append(np.random.rand())
    ...
    def __getitem__(self, index):
        ...
        crandom = self.random_idx[index]
        # print(crandom, yo.sum(), yo.shape)
        if self.aug:
            if crandom < 0.1:
                yo = apply_morphological_operations(yo.unsqueeze(0), operation='dilation', kernel_size=3, iterations=1).squeeze()
            #    #   print(iter, 1)
            elif crandom > 0.9:
                yo = apply_morphological_operations(yo.unsqueeze(0), operation='erosion', kernel_size=3, iterations=1).squeeze()
        # print(crandom, yo.sum())
        # # mask.shape - torch.Size([4])
        # # x.shape - torch.Size([4, 80, 80, 80]); yo.shape - torch.Size([4, 80, 80, 80])
